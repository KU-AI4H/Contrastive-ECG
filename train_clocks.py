...
import numpy as np
import mlflow
from models import ECG_CNN_Encoder, ECG_CLOCKS_Classifier
from utils import *
from CL_augmentations import *


def main_train_test(pretrain_signals, pre_train_labels, data_df, embedded_size=128, kernel_size=15, dropout=0.1, CL_embedded_size=64, pretrain_num_epochs=50, num_epochs=70, print_results = False, save_results = False, plot_results = False, seed=42, cl_temp=0.07, augmentation1='dropout',  aug1_ratio=0.20, augmentation2='guassian_noise', aug2_ratio=0.15, domain ='time' , exp_name=None, encoder_name='CL_encoder', pre_batch_size=128, batch_size=12, pretrain_encoder_path=None, pretrain=True, agg_func='cat'):
    ...
    train_patients, temp_patients = train_test_split(
        ...
        test_size=0.4,  # 20% goes to (val + test)
    ...
    if pretrain: 
        ...
        if pretrain_encoder_path==None:
            ...
            encoder_path = f"{encoder_name}_clocks_CMSC_aug1_{augmentation1}_ratio_{aug1_ratio}_agu2_{augmentation2}_ratio_{aug2_ratio}_domain_{domain}_epoch_{pretrain_num_epochs}_num_{len(pretrain_signals)}_batchsize_{pre_batch_size}_clemb_{CL_embedded_size}_cltemp_{cl_temp}.pth"
        ...
        if os.path.exists(encoder_path):
            ...
            for epoch in tqdm(range(pretrain_num_epochs)):
                ...
                for signals, _ in pretrain_loader:
                    ...
                    first_half_signals, second_half_signals = nonoverlap_time_segmenting(signals)

                    pretrain_optimizer.zero_grad()

                    aug_first_half_signals1 = aug_function(first_half_signals, augmentation1, aug1_ratio, seed=seed)
                    aug_first_half_signals2 = aug_function(first_half_signals, augmentation2, aug2_ratio, seed=seed+5)

                    aug_second_half_signals1 = aug_function(second_half_signals, augmentation1, aug1_ratio, seed=seed)
                    aug_second_half_signals2 = aug_function(second_half_signals, augmentation2, aug2_ratio, seed=seed+5)

                    _, aug_repr1_first_half = encoder(aug_first_half_signals1)
                    _, aug_repr2_first_half = encoder(aug_first_half_signals2)
                    
                    _, aug_repr1_second_half = encoder(aug_second_half_signals1)
                    _, aug_repr2_second_half = encoder(aug_second_half_signals2)

                    contrastive_loss_first_half = cl_loss_fn(aug_repr1_first_half, aug_repr2_first_half)
                    contrastive_loss_second_half = cl_loss_fn(aug_repr1_second_half, aug_repr2_second_half)
                    contrastive_loss_first_second_half = cl_loss_fn(aug_repr1_first_half, aug_repr2_second_half) + cl_loss_fn(aug_repr1_second_half, aug_repr2_first_half)

                    contrastive_loss = contrastive_loss_first_half + contrastive_loss_second_half + contrastive_loss_first_second_half
    ...
    classifier = ECG_Classifier(encoder, embedded_size=embedded_size, dropout=dropout, agg_func=agg_func).to(device)
...
def cross_val_main(pretrain_signals, pre_train_labels, data_df, print_seed_results=True, record_seed_results=True, pretrain_num_epochs=50, num_epochs=60, embedded_size=64, kernel_size=15, dropout = 0.1, CL_embedded_size=64, domain='time', cl_temp=None,  augmentation1='dropout',  aug1_ratio=0.20, augmentation2='guassian_noise', aug2_ratio=0.15,  encoder_name='CL_encoder', pre_batch_size=128, batch_size=128, pretrain_encoder_path=None, pretrain=True, agg_func='cat'):
    ...
    for seed in seeds:
        ...
        best_metrics, best_test_metrics = main_train_test(pretrain_signals, pre_train_labels, data_df, embedded_size=embedded_size, kernel_size=kernel_size, dropout=dropout, CL_embedded_size=CL_embedded_size, pretrain_num_epochs=pretrain_num_epochs, num_epochs=num_epochs, print_results = False, save_results = False, plot_results = False, seed=seed, cl_temp=cl_temp,  augmentation1=augmentation1,  aug1_ratio=aug1_ratio, augmentation2=augmentation2, aug2_ratio=aug2_ratio, domain =domain , exp_name=None, encoder_name=encoder_name, pre_batch_size=pre_batch_size, batch_size=batch_size, pretrain_encoder_path=pretrain_encoder_path, pretrain=pretrain, agg_func=agg_func)
