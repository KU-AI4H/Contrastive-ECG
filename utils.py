...
#     def forward(self, repr, new_repr):

#         device = repr.device
#         batch_size = repr.shape[0]

#         normalized_repr_embed = F.normalize(repr, p=2, dim=1)
#         normalized_new_repr_embed = F.normalize(new_repr, p=2, dim=1)

#         # Cosine similarity
#         sim_matrix = torch.matmul(normalized_repr_embed, normalized_new_repr_embed.T) / self.temperature # shape = (batch_size, batch_size)

#                 # Create labels for positive pairs (diagonal = 1)
...
class NTXentLoss(nn.Module):
    ...
    def forward(self, repr, new_repr):
        ...
        Args:
            repr (torch.Tensor): First augmented batch of embeddings (shape: [batch_size, feature_dim]).
            new_repr (torch.Tensor): Second augmented batch of embeddings (shape: [batch_size, feature_dim]).
        ...
        new_repr = F.normalize(new_repr, p=2, dim=1)

        # Concatenate both representations (2N, D)
        z = torch.cat([repr, new_repr], dim=0)  # Shape: (2*batch_size, feature_dim)

        # Compute cosine similarity matrix (2N x 2N)
        sim_matrix = torch.matmul(z, z.T) / self.temperature  # Similarity matrix

        # Create positive pair indices (each sample's correct match)
        labels = torch.arange(batch_size, device=device)
        labels = torch.cat([labels + batch_size, labels], dim=0)  # Shape: (2*batch_size,)

        # Apply mask to remove self-similarity (diagonal elements)
